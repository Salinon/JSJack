...
var eg = {
	...
	bet : function() {
		...
		do {
			...
			if (les.lessonList[les.currLesson].riggedDeck) {
				...
				if (!les.lessonList[les.currLesson].reshuffle || !eg.deck) {
					eg.deck = cards.shuffle(cards.makeDeck());
					eg.reshuffled = true;
				} else if (eg.deck.length < 11) {
					eg.deck = cards.shuffle(cards.makeDeck());
					eg.reshuffled = true;
				} else {
					eg.reshuffled = false;
	...
	hit : function() {
		...
		// Check for bust
		if (eg.playerHand.length >= 5 || eg.value(eg.playerHand) > 21) {
			eg.end();
		}
	},
	stand : function() {
	...
	// The game has ended, for whatever reason
		...
		if (eg.value(eg.playerHand) > 21) {
			...
			eg.win("Dealer bust!");
		} else if (eg.playerHand.length >= 5) {
			eg.win("You win!");
	...
	win : function(msg) {
		$("#gameOut").show();
		$("#gameOut").html("<img src='./img/win.png' alt='You win!' style='position:absolute' />").delay(1500).fadeOut();
		eg.money += eg.betValue;
		cb.call('won');
	},
	lose : function(msg) {
		$("#gameOut").show();
		$("#gameOut").html("<img src='./img/lose.png' alt='You lost!' style='position:absolute' />").delay(1500).fadeOut();
		eg.money -= eg.betValue;
	},
	// Double down
	doubleDown : function() {
		...
		var animation = window.setInterval(function() {
			...
			try {
				if (eg.turns > turncap || !eg.inGame) {
					...
					eg.turns = -1;
					eg.evalLocked = false;
					if (les.lessonList[les.currLesson].GRAWWR) {
						try {
							jQuery.globalEval(code);
							cb.call('GRAWWR');
						} catch (E) { // Jesus christ...
